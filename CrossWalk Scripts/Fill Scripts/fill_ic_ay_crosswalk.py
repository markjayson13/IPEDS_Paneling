...
DEFAULT_TEMPLATE = PROJECT_ROOT / "Paneled Datasets" / "Crosswalks" / "ic_ay_crosswalk_template.csv"
...
def suggest_concept_key(row: pd.Series) -> str | None:
    ...
    var = (row.get("source_var") or "").strip().upper()
    ...
    if var == "CHG1":
        if "tuition" in label and "district" in label and ("fee" in label or "required" in label):
            return CHG_CONCEPTS[var]
        if "in-district" in label and "tuition" in label:
            return CHG_CONCEPTS[var]
    ...
    if var == "TUITION1":
        if "tuition" in label and "in-district" in label and "full-time" in label:
            return TUITION_FEE_CONCEPTS[var]
...
def apply_suggestions(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    mask_empty_before = df["concept_key"].astype(str).str.strip() == ""
    ...
    stats = {
        ...
        "n_unfilled": int((df["concept_key"].astype(str).str.strip() == "").sum()),
    }
    return df, stats
...
def print_summary(df: pd.DataFrame, stats: Dict[str, int]) -> None:
    ...
    unfilled_mask = df["concept_key"].astype(str).str.strip() == ""
...
def main() -> None:
    ...
    df = load_template(args.template)
